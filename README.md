...
*Five interventions deployed with the ordinary `set_*()` builders — treatment scale-up, RTS,S via EPI, seasonal SMC, IRS and bed nets. Four of the five agree to within two percentage points of reduction. Seasonal SMC is the exception: blink cuts under-5 clinical incidence by 40% against the IBM's 52%, because it decays SP-AQ prophylaxis exponentially at the Weibull mean, which lets protection leak between monthly rounds (see the article).*
