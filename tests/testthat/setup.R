...
#
# Escape hatch: set the environment variable FLEET_ALLOW_SKIP to any non-empty
...
if (!requireNamespace("malariasimulation", quietly = TRUE) &&
    !nzchar(Sys.getenv("FLEET_ALLOW_SKIP"))) {
  stop("'malariasimulation' is not installed, so nearly every test in this suite ",
       ...
       "Install it (Remotes: mrc-ide/malariasimulation), or set FLEET_ALLOW_SKIP=1 ",
