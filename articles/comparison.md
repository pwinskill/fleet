...
> separate harness called `blink2_validate` that is **not part of this
> repository and is not public**. There is no `blink2_validate`
...
`blink2_validate`, a separate harness that is not part of this
repository and is not public: there is no repository to clone and no way
for a reader to run it. Its results reach this page only through the
committed snapshot `comparison/data/site_snapshot.json`, which
`summary_tables.R` reads for the statistics, and the committed
`cmp_core_sites.png`. Neither is regenerated by an ordinary run of the
scripts above; both are re-taken deliberately, with
`CMP_REFRESH_SITES=1`, and only on a machine that has the validation
results. That snapshot and that figure are the whole public record of
the comparison.
