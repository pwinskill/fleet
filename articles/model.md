...
|----|----|----|----|----|
| Age group | $`i = 1,\dots,n_a`$ | 118 | `age_lower =` / [`default_age_lower()`](https://pwinskill.github.io/fleet/reference/default_age_lower.md) | Graded age grid, fine in childhood and coarse in adulthood. Group edges are pinned at the conventional reporting ages (0, 1, 2, 3, 5, 7, 10, 15, 20, 30, 40, 60 y), so output bands fall on group boundaries, and at 21 y, so that whole groups make up the year maternal immunity’s mothers are drawn from (§B.3); between two anchors the groups are of equal width, with the budget of groups shared out in proportion to **log** width, because immunity rises with log age rather than with age, weighted 1.25 below 5 y and 0.5 from 21 y, where the grid’s error does and does not arise. An absorbing top group runs from 80 y. Individuals age by a linear-chain (McKendrick) flow $`r_i`$ between adjacent groups. |
...
**Total state size.** With the defaults ($`n_a = 118`$, $`n_z = 5`$,
$`n_v = 1`$) and no drugs, so that every chain has one stage, there are
10,030 falciparum human states,
...
$`n_a n_z = 590`$ states. The vivax block (§V) multiplies its
...
median, severe incidence at EIR 120 runs 7% low on 53 groups and 2% on
118, the default.
[`vignette("using")`](https://pwinskill.github.io/fleet/articles/using.md)
