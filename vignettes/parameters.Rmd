...
| Vivax-only | All hypnozoite parameters, `drug_hypnozoite_*`, `n_with_hypnozoites_rendering_*` | ✅ Used under `parasite = "vivax"` |
...
| &nbsp;&nbsp;`drug_hypnozoite_*` | *P. vivax* only | ✅ Radical cure: batch reset for the treated, and liver-stage protection as an Erlang chain of up to five stages from the Weibull |
...
| `n_with_hypnozoites`, `hypnozoites`, `iaa`, `iam` | *P. vivax* only | ✅ `n_with_hypnozoites` and `n_relapses` are output columns (0 under falciparum); the immunity means are not rendered, as for falciparum |
