...
# This article is prose, equations and tables only. The R shown in section 12 is
...
space does and does not carry, §11 covers every `malariasimulation` argument one
by one, and §12 lists what to do differently because this is an ODE and not the
IBM. If you read only one section, read §12.
...
(§7), then resumes. This keeps chemoprevention out of the ODE right-hand side
entirely.
...
the hazard, while the boosting terms of §4.5 keep the deduplicated event
...
for the slow-parasite-clearance fraction of the treated (§11.8), and
...
- **$P_c$ has no ODE inflow.** It is filled only by the chemoprevention pulses
  of §7, and carries neither infection risk nor infectivity.
...
the drift to well under 1% (no setting makes it exactly zero — see §4.2).
...
fixed point (§8), and a seasonal run needs ~10 years to settle onto its limit
...
equilibrium age structure the whole run is seeded on (§11.5).
...
person-day-weighted aggregate (§9). Set each family's own fields:
...
equilibrium test, where it cuts the drift off the seed to well under 1% (§4.2).
...
no effect — `blink` re-derives `total_M` from `init_EIR` (§11.4).
...
re-vaccination exclusion is not applied (§11.16, §11.17). A single campaign is
fine.

### Cost model

Run time scales with $n_a \times n_z$ (the human state), and — importantly —
with the **number of chemoprevention pulses**, because §7 splits the integration
