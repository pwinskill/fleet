...
post-treatment and one for chemoprevention, each drawn as a stack of boxes to mark
it as an Erlang chain with its stage count beside it. Its deck is labelled for the
52 age groups, and a note records that every box is also an array over the five
biting-heterogeneity nodes, that people age between layers, die from every box, and
are born into susceptible. A dotted region encloses the four states infectious to
mosquitoes; a dotted ring round the susceptible, asymptomatic and sub-patent boxes
marks the states the infection hazard acts on. The treated state is drawn once; the
...
asymptomatic; recovery returns everyone to susceptible. A small box holds the four
immunity arrays, coupled into the hazard. A dashed red path marks the
...
`set_mda(parameters, drug, timesteps, coverages, min_ages, max_ages)` — and
`set_smc()` with the identical signature and identical handling.
...
projections,
`atol = rtol = 1e-6` with `step_size_max = 10` runs several times faster with
negligible effect on aggregate outputs.
