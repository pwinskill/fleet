...
susceptible; untreated clinical disease; two treated compartments for standard and
slow parasite clearance; asymptomatic and sub-patent infection; and two prophylaxis
compartments, one post-treatment and one for chemoprevention. A dotted region
encloses the five states that are infectious to mosquitoes. The infection hazard
acts on the susceptible, asymptomatic and sub-patent states together and splits four
ways; recovery returns everyone to susceptible. A dashed arrow marks the
...
- **Humans, above.** The infection hazard $\Lambda$ acts on the whole at-risk pool
  ...
  four ways: untreated clinical disease $D$, one of the two treated compartments
  $T$ / $T_s$, or a non-clinical infection landing in $A$. Recovery runs
  $D \to A \to U$, and everyone returns to $S$ — from $U$ directly, from $T$ /
  $T_s$ via prophylaxis $P$, and from chemoprevention prophylaxis $P_c$.
...
Three things the figure leaves out, all of them state the model does integrate: the
four immunity arrays $I_B$, $I_{CA}$, $I_D$, $I_{VA}$, which set $b$, $\phi$, $q$ and
$\theta$ on the arrows drawn; the three Erlang lag chains behind the two couplings
and the incubation period; and the age × heterogeneity structure inside every human
box. All three are the subject of §3. Nor does it mark where interventions act —
with the exception of chemoprevention, which is the one intervention that adds a
