...
- **Custom demography: mosquito sizing now replicates
  ...
  63-country validation set this release moves blink’s mean
...
- **`test-prophylaxis-chain.R`’s first test no longer errors when
  ...
  suite’s 57 test blocks to do so. `malariasimulation` is a GitHub
  Remote in Suggests, so on any check machine that could not install it
  the other 56 blocks skipped and this one errored, turning a missing
  optional dependency into an `R CMD check` failure. The block is now
  split rather than simply guarded: its closed-form `erlang_stages()`
  checks need no drug tables and keep running unguarded, so the suite
  retains real assertions without the dependency, and only the Weibull
...
- The intervention-impact figure gains two panels — clinical incidence
  ...
  *who* each intervention protects: SMC and RTS,S roughly halve their
  ...
  and IRS read the same either way. blink lands inside the IBM’s 10–90%
...
- The country-site comparison figure reads its 1:1 correlation properly
  ...
  ramp, and not a series colour, so it cannot be misread as a model. And
  the low end of the hex fill ramp is now near-white: the counts are
  wildly skewed, with 52% of the cells carrying 0.2% of the
  sub-site-months while the top 5% of cells carry 90% of them, so the
  old saturated low end spent most of the plot’s ink on almost none of
  the data and buried the ridge it was meant to show. The scale is still
  log10, so sparse cells stay visible — the scatter is real and worth
...
- Seeded at the `malariaEquilibrium` fixed point; holds flat at
  equilibrium to machine precision, including under clinical treatment
...
- [`run_simulation_ode()`](https://pwinskill.github.io/blink/reference/run_simulation_ode.md)
  exposes the dust2 solver controls (`atol`, `rtol`, `step_size_max`)
...
- Compared against `malariasimulation` across PfPR–EIR, age-prevalence
  ...
  clinical & severe incidence time series (see `comparison/`). LM
  prevalence and clinical incidence agree with the IBM to within ~1%.
...
- *P. falciparum* only; *P. vivax* and the individual-mosquito code path
  ...
  are indicative (can differ from the IBM by ~5–20%, largest at low
  EIR).
